...
#4 Impossible
'''
# 문제 설명에 부족한 부분이 많아도 잘 읽어보며 문제를 파악하기....
T = int(input())
for tc in range(1, T + 1):
    N, M, K = map(int, input().split())
    time = sorted(list(map(int, input().split()))) # 몇초에 손님이 방문하는지 순서대로 정렬
    # 손님이 도착하는 시간과 붕어빵을 만드는 시간을 비교해 손님이 방문했을때 재고 확인
    for i in range(0, N):
        if (time[i] // M) * K < i + 1:
            print(f'#{tc} Impossible')
            break
    else: # 손님이 다 가도 붕어빵재고가 부족하지 않으면 Possible
        print(f'#{tc} Possible')
