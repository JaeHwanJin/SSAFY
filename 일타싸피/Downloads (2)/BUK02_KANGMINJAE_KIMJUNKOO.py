...
NICKNAME = 'BUK02_KANGMINJAE_KIMJUNKOO'
...
def check_dist(start, end):
    check_list = [*balls[1:], *HOLES]
    path_x = end[0] - start[0]
    path_y = end[1] - start[1]
    len_path = math.sqrt(path_x**2 + path_y**2)
    path = [path_x, path_y]
    for check in check_list:
        vector_x = check[0] - start[0]
        vector_y = check[1] - start[1]
        vector = [vector_x, vector_y]
        if dot(path, vector) < 0:
            print('반대에 있다')
            continue
        dist = cross(path, vector) / len_path
        if dist < size_ball * 1.05:
            print('경로에 공이 있다.')
...
def play():
    ...
    for ball in tballs:
        if (ball[0] == -1.0) and (ball[1] == -1.0):
            continue
        for hole in HOLES:
            temp_destination = find_destination(ball, hole)

            dif_angle = math.fabs(cal_angle(ball, hole) - cal_angle(whiteBall, temp_destination))
            if min_dif > dif_angle:
                min_dif = dif_angle
                min_destination = temp_destination

    t_distance = cal_distance(whiteBall, min_destination)
    angle = cal_angle(whiteBall, min_destination)
    if t_distance > 20:
        power = t_distance * 0.66
    else:
        power = 50.0   
